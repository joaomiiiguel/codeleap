...
import { logginUser, logoutUser, setShowModalAlert, setShowModalDelete, setShowModalEdit } from '../redux/UserSlice'
    ...
    <>
      {
        userName === null || userName === '' ?
          ...
          <main className="flex min-h-screen flex-col items-center space-y-6 bg-background text-black pb-8">
            <div className='flex flex-row items-center justify-between w-full bg-blue text-white py-6 px-[8%] md:px-[15%]'>
              <p className='font-bold text-xl'>CodeLeap Network</p>

              <IconButton className='text-white' aria-label="Logout User" onClick={() => dispatch(logoutUser(), localStorage.clear())}>
                <LogoutIcon size={20} />
              </IconButton>
            </div>
            <div className='flex flex-col w-full px-[8%] md:px-[15%] max-w-[1000px] space-y-4 mb-10'>
              <NewPostComp />
            </div>
            <div className='flex flex-col w-full px-[8%] md:px-[15%] max-w-[1000px] space-y-4 mb-10 scroll-pl-6 snap-y'>
              {
                loadingListPosts ?
              ...
              <div className='flex w-full justify-center z-50' id="check">
                {loadingPaginationPosts ?
                  ...
                  <div className='flex flex-col w-full justify-center text-center opacity-60'>
                    {dataPost.count > 10 && <Button variant="text" color="inherit" size='small' fullWidth onClick={() => getPostWithPagination()}>See more...</Button>}
                  </div>
                }
              </div>
            </div>
